...
/* harmony import */ var primereact_speeddial__WEBPACK_IMPORTED_MODULE_15__ = __webpack_require__(56002);
/* harmony import */ var primereact_toast__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(74420);
/* harmony import */ var primereact_dialog__WEBPACK_IMPORTED_MODULE_16__ = __webpack_require__(78052);
/* harmony import */ var primereact_badge__WEBPACK_IMPORTED_MODULE_18__ = __webpack_require__(45795);
/* harmony import */ var nanoid__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(33220);
/* harmony import */ var react_bootstrap__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(34051);
/* harmony import */ var react_icons_fa__WEBPACK_IMPORTED_MODULE_19__ = __webpack_require__(91871);
...
/* harmony import */ var react_bootstrap_Card__WEBPACK_IMPORTED_MODULE_14__ = __webpack_require__(25692);
/* harmony import */ var _reach_router__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(87896);
/* harmony import */ var gatsby__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(57076);
/* harmony import */ var react_fontawesome__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(75980);
...
*/async function handlePost(id){const nano=(0,nanoid__WEBPACK_IMPORTED_MODULE_9__/* .nanoid */ .x0)();setCardreply(cardreply=>({...cardreply,[id]:true}));handleClear();try{const snapshot=await app1515.database().ref(`Replies/`).orderByChild("ip").equalTo(id).once("value");if(snapshot.exists()){await app1515.database().ref(`Replies/`).update({replied});}else{const Replies=app1515.database().ref(`Replies/`);Replies.set({replied});}}catch(error){console.log(error);}validated();}/*



*/ /* Limit height of Box item */ /*end of Limiting Box size */(0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{if(user){const commentsRef=app1515.database().ref(`Replies/`);const commentsListener=commentsRef.on('value',snapshot=>{if(snapshot.exists()){const commentsArray=Object.values(snapshot.val().replied);const editingObject=commentsArray.reduce((acc,card)=>{acc[card.id]=true;return acc;},{});setCardreply(editingObject);setReplied(commentsArray);}});// Return a cleanup function to remove the listener when the component unmounts or when the `user` state changes.
...
alert("reported");}}];//Thumbs Construct end
...
if(!updatedRates.some(rate=>rate.cardId===cardId&&rate.user===props.user)){updatedRates.push({cardId,user:props.user,liked:false,disliked:true,ip:id,date:new Date(),userId:user.uid});}return updatedRates;});try{const snapshot=await app1515.database().ref('Reply').orderByChild('cardId').equalTo(cardId).once('value');if(snapshot.exists()){const ratingKey=Object.keys(snapshot.val())[0];// assuming there's only one matching rating
const rating=snapshot.val()[ratingKey];await app1515.database().ref(`Reply/${ratingKey}`).update({disliked:!rating.disliked,liked:false});}else{const userRef=app1515.database().ref('Reply');await userRef.push({cardId,user:props.user,liked:false,disliked:true,ip:id,userId:user.uid,date:new Date()});}}catch(error){console.error('Error updating:',error);}}// Function to handle the user's like action
...
const generateMarkup=()=>{const words=htmlString.split(' ');const elements=[];words.forEach((word,index)=>{if(word.startsWith('http')||word.startsWith('https')){const element=handleImageOrVideos(word)||/*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_1___default().createElement("a",{href:word,target:"_blank",rel:"noopener noreferrer"},word);elements.push(element);}else if(word.startsWith("data:image/")){// If the word is a base64-encoded image data URL, create an <img> tag
elements.push(`<div class="card flex justify-content-center"><Image className="sized" src="${word}" zoomSrc="${word}" alt="Image" class="resizedImages" width="250" preview /></div>`);}});return elements.join('');};const resized=()=>{const htmlStrin=generateMarkup();const regex=/<pre [^>]*>/g;const matches=htmlStrin.match(regex)||[];let modifiedHtmlString=htmlStrin;for(const match of matches){const newTag=`<pre class="d-block w-40" id="resized" > ${match} </pre>`;modifiedHtmlString=modifiedHtmlString.replace(match,newTag);}return modifiedHtmlString;};const{0:zoom,1:setzoom}=(0,react__WEBPACK_IMPORTED_MODULE_1__.useState)(false);const{0:edrefs,1:setedref}=(0,react__WEBPACK_IMPORTED_MODULE_1__.useState)(true);const edref=(0,react__WEBPACK_IMPORTED_MODULE_1__.useRef)(null);const setFullSizeImage=props.setFullSizeImage;const fullSizeImage=props.fullSizeImage;const editRef=(0,react__WEBPACK_IMPORTED_MODULE_1__.useRef)(null);const{0:useref,1:setuseRef}=(0,react__WEBPACK_IMPORTED_MODULE_1__.useState)(true);const editHeight=(_editRef$current=editRef.current)===null||_editRef$current===void 0?void 0:(_editRef$current$getB=_editRef$current.getBoundingClientRect())===null||_editRef$current$getB===void 0?void 0:_editRef$current$getB.height;if(editRef.current){if(editHeight>=120){editRef.current.style.height="60vh";edref.current.style.display="flex";}else{editRef.current.style.height="fit-content";edref.current.style.display="none";}}(0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{const images=document.querySelectorAll(".resized-imaged");images.forEach(image=>{image.addEventListener("click",handleImageClick);return()=>{image.removeEventListener("click",handleImageClick);};});},[]);const debouncedHandleChange=lodash_debounce__WEBPACK_IMPORTED_MODULE_0___default()(event=>{handleEd(event);},300);const edits=id=>{edited(id);debouncedHandleChange(id);};function handleEd(id){if(editRef.current!==null&&id===editRef.current.id){console.log("it's fogginggggggggggggggggggggg second");editRef.current.style.height='100%';}}function edited(id){console.log("it's fogginggggggggggggggggggggg first");if(editRef.current!==null&&id===editRef.current.id){console.log("it's fogginggggggggggggggggggggg third");editRef.current.style.height='100%';setedref(false);}}function handleImageClick(event){if(useref===true){const clickedEl=event.target;if(clickedEl.tagName==="IMG"){// Do something with the clicked image
setFullSizeImage(clickedEl.src);}}}function resizeImage(){const htmlStrin=generateMarkup();const regex=/<img [^>]*>/g;const reg=/<pre [^>]*>/g;const matches=htmlStrin.match(regex)||[];let modifiedHtmlString=htmlString;for(const match of matches){const widthRegex=/width="(\d+)"/;const heightRegex=/height="(\d+)"/;const srcRegex=/src="([^"]+)"/;// add regex to extract src attribute
...
const newTag=`<Image src="${src}" id="resizedImages" style="width:100%; max-width:300px" class="resizedImages p-image-preview p-image-mask" width="250" preview  />`;const ma=htmlStrin.match(reg);modifiedHtmlString=modifiedHtmlString.replace(match,`<div class=" p-image-preview-container card flex justify-content-center imagewraper d-flex-column">${newTag}</div>`);}return modifiedHtmlString;}const ifZomed={position:zoom&&"absolute",top:zoom&&"0",left:zoom&&"0",right:zoom&&"0",bottom:zoom&&"0",zIndex:zoom&&"20",backgroundColor:zoom&&"rgba(0,0,0,0.11)"};/*
...
*/(0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{if(user){const commentsRef=app1515.database().ref(`commentpost/`);commentsRef.on('value',snapshot=>{if(!snapshot.val()){return;}const commentsArray=Object.values(snapshot.val());setCards(commentsArray);});// cleanup function to remove the listener when the component unmounts
return()=>commentsRef.off('value');}},[user]);(0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{if(props.posted&&typeof props.posted!=="object"){const resizedHtmlString=resizeImages(props.posted);setHtmlString(resizedHtmlString);}else if(props.posted&&typeof props.posted==="object"){setHtmlString(`<div>
    ...
    <h5>${props.posted.title}</h5>
      <img src=${props.posted.images[3]?props.posted.images[3]:_images_no_image_jpg__WEBPACK_IMPORTED_MODULE_11__/* ["default"] */ .Z} />
      ...
      <img class="bgimg" src=${props.copied[0].message.images[3]?props.copied[0].message.images[3]:_images_no_image_jpg__WEBPACK_IMPORTED_MODULE_11__/* ["default"] */ .Z} />
      <h1 class="bgh1">${props.copied[0].message.title}</h1>
      <p class="bgp">${props.copied[0].message.paragraphs}</p>
    </div>`);}},[props.copied]);//show the user there upvotes
/*
