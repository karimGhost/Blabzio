...
const [user2, setUser2] = useState('');
  ...
  return (
    <div className='container containersmall Signup' fluid>
<Toast ref = {msgs} />
      ...
      <Card className='mx-5 mb-5 p-5 shadow-5 ' id="cardSign" style={{marginTop: '', backgroundImage: `${img}`, backdropFilter: 'blur(30px)'}}>
        ...
        <Card.Body className='p-4 text-center psmall'>
        
        <div className='d-flex flex-row ps-5 pb-2' style={{ justifyContent:"center"}}>
            <FaDragon  fas icon=" fa-3x me-3 " style={{color:"purple", divor: '#709085' }}/>
         


            <span className="h1 fw-bold mb-0 text-center" style={{color:"whitesmoke"}}  >Blab<b style={{color: "purple", fontFamily:"sans-serif"}}>Z</b>io </span>
          </div>     
               <h2 className="fw-bold mb-5">Sign up </h2>
          {error && (
        <p className="error-messag">{error}</p>
      )}   
      <form onSubmit={handleSubmit} style={{width:"fit-content"}} className=' pl-5 justify mx-auto  ml5'>
          <Row className='rowed ' >
...
<Password  placeholder='password' style={{width: "100%"}} className=' passs mb-4 pr-1 center  pl-5 pls5' id='form2' value={Logindata.pass1}  onChange={(event) => setLoginData(pre => {return{ ...pre, pass1: event.target.value }} ) } toggleMask />
...
<Password className='passs mb-4 pl-5 pls5' placeholder='repeat-password '  style={{width: "100%"}} id='form3' value={Logindata.pass2}  onChange={(event) => setLoginData({...Logindata, pass2: event.target.value}) } toggleMask   footer={footer}/>
          ...
          <Button  type="submit" style={{background: "rgb(20,21,21)", width: "90%"}} className='w-50 mb-4 ' size='md'>sign up</Button>
...
<div className='d-flex flex-row ps-5 pb-2 ' style={{ justifyContent:"right", fontSize:"1.5rem"}}>
            <FaDragon  fas icon=" fa-3x me-3 " style={{color:"purple", divor: '#709085',fontSize:"0.95rem" }}/>
         


            <span className="h1 fw-bold mb-0 text-center " style={{color:"whitesmoke" ,fontSize:"1.5rem"}}  >Blab<b style={{color: "purple", fontFamily:"sans-serif",fontSize:"1.5rem"}}>Z</b>io </span>
          </div>     
</div>
        </Card.Body>
      </Card>
