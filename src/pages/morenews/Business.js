...
import { faArrowsLeftRightToLine, faChevronLeft, faChevronRight, faDownload, faHeart, faSave } from "@fortawesome/free-solid-svg-icons";
...
export default function Business() {
  ...
  firebase.initializeApp(firebaseConfig1412, 'app1312');
  ...
  const app1919 = firebase.initializeApp(firebaseConfig1719, 'app1819');
...
function handleLanguageChange(event) {
  ...
  */
  ...
  if(!news){
    return <Loadings  head={<Trans>Business</Trans>}  />
  }
  return (
    <Layout isBloged={isBloged} setDark={setDark} setLight={setLight} light={setLight} lag={lag} off={off} handleLanguageChange ={handleLanguageChange} language={language}>
    <PagesData light={light} setLight= {setLight} off={off} setoff={setoff}  da={da} />



    <div className="container-scrolle " >
      <div className= 'main-pane '>

    <div className={`flash-news-banner ${light && "back"}`}>
    <Market />
                   ...
                   { currentNews && currentNews.slice(0, 10).map((newss, index) => ( 
                      <div  key={index} className="row">
                        <div className="col-sm-4 grid-margin">
                          <div onClick={() =>  locale(newss.url)} className="rotate-img">
                            <img
...
{ currentNews && currentNews.slice(10, 20).map((newss, index) => (
      ...
      > <FontAwesomeIcon icon={faDownload} className="savedit" style={  { color: saved[newss.title] ? "tomato" :  "yellowgreen"}} /> {saved[newss.title] ? 'saved' : "save"} </button>  
                    ...
                    <div className="col-lg-4">
                      ...
                      { currentNews && currentNews.slice(20, 30).map((newss, index) => (
      ...
      ><FontAwesomeIcon icon={faDownload} className="savedit" style={  { color: saved[newss.title] ? "tomato" :  "yellowgreen"}} /> {saved[newss.title] ? 'saved' : "save"} </button>  
                              <div className="col-sm-4"   onClick={() =>  locale(newss.url)}>
                                <div className="rotate-img"   onClick={() =>  locale(newss.url)}>
                                  <img
                                    src={newss.urlToImage}
                                    alt="banner"
...
export const Head = () => <title>Business</title>
