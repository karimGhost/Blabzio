...
{!props.shared && 
  <div className={`py-1 `} >
    <Row style={{background: "transparent"}} className="justify-content-center ">
      <div style={{width:"100%"}} md="12" lg="10" xl="10" className=" w-100 p-0 " >
        <Card id="wsmall" className={ `"p-0  carded  " ${props.dark ? "bb" : "pp"}`}  style={{  color: props.dark ? "silver" : "black", background: props.dark ? "#141515"  :  "white", width:"65%", padding: "0", marginLeft:"25%"}}>
          ...
          <Card.Body>
      { props.user === username ?
          <SpeedDial showIcon="" outlined  buttonClassName={`${ isOpen ? 'pi pi-times' : 'pi pi-ellipsis-h' } p-button-warning bold border-non p-button-outlined    sm   p-mt-2 p-0 p-2`} model={items}  visible={isOpen}
    onShow={handleToggle}
...
<Button label="TopRight" icon="pi pi-arrow-down-left" onClick={() => show('top-right')} className="p-button-warning" style={{ minWidth: '10rem' }} /> */}
            ...
            <Link to={props.username === props.user ? `/profileComponents/Myprofile` :
             props.username ?`/profileComponents/Profiles?id=${props.userId}` : ""}>  
            ...
            <Card.Img
              ...
              /></Link>
              <div>
              <Link to={ props.username === props.user ? `/profileComponents/Myprofile` : props.username ?  `/profileComponents/Profiles?id=${props.userId}` : ""}>  
...
<div style={{width: "100%", background: props.dark &&  "#141515"} } >
     ...
     <div style={{alignItems:"center", width: "50%", margin: "auto",  background: props.dark ?  "#141515" : 'rgba(0,0,0,0.11)'}}>
            <p style={{textAlign : "center", color: "green", fontWeight:"bold", fontFamily:"cursive", background: "black"}}>reposted content</p>
             ...
             <Link to={`/profileComponents/Profiles?id=${props.copied[0].userId}`}>     <Card.Img
              ...
              /></Link>
              <div style={{display: ""}}>
              <Link to={`/profileComponents/Profiles?id=${props.copied[0].userId}`}>  <h5  className="fw-bold text-dark  mb-1">{copied[0].user}</h5></Link>
