...
import Postit from '../news/Posted/Postit';
...
useEffect(() => {  
    ...
    async function dataset(){
      ...
      try {
       ...
       if (snapshot.exists()) {
        ...
        const userRef =   app09.database().ref('profile/' + user.uid)
  ...
  useEffect(() => {
    ...
    if (user) {
      app09.database().ref(`profile/${user.uid}/profileImg`).on('value', snapshot => {
      ...
      app09
        ...
        .then((snapshot) => {
          if (snapshot.exists()) {
            ...
            const snapshotval = snapshot.val()[userId];
            if (snapshotval) {
              const userRef = app09.database().ref(`profile/${userId}`);
              userRef.update({
                text: Bio.text,
                status:Bio.status,
...
useEffect(() => {
  ...
  const getFollowers = async () => {
    ...
    const snapshot = await app09.database().ref(`profile/${user.uid}/followers`).once('value');
...
const snapshot = await app09.database().ref(`profile/${user.uid}/following`).once('value');
...
if(user){
  const getBlock = async () => {
    ...
    const ref =  app09.database().ref(`profile/${user.uid}/blockList`);
...
return () => {
app09.database().ref(`profile/${user.uid}/blockList`).off('value');
};
...
<section className={ ` ${dark && "darken"}  bg-light h-100 gradient-custom-2 mt-8 bgsm`}>
  ...
  <div className="container  py-5 h-100">
    <div className="row  d-flex justify-content-center align-items-center h-100">
      <div className="col  col-lg-9 col-xl-7">
        <div className="car ">
          ...
          <div className="tops rounded-top text-white d-flex  flex-row bg-light" style={{backdropFilter:"blur(15px)", position: "relative",height: "100px",  backgroundImage:  `url(${profileImg})`, height:"200px",  objectFit:"cover",background: gradient ,  backgroundSize: "contain", marginTop: "90px"}}>
          ...
          {hideColors && <div>
           ...
           <div style={{position: "absolute", right: "0", top: "150px"}} className="card flex justify-content-center">
            ...
            <SplitButton className='bg-danger pi pi-arrow-left z-index-4 rdd h-1 ' buttonClassName=" bg-transparent b-transparent transparent w-1 p-0 m-0" severity="warning" outlined style={{ backgroundColor:"yellow" ,zIndex:"10", color: "red", fontSize: "5px", padding:"1px" }} size="small" label=""  icon="pi pi-arrow-left"  model={items} />
        </div>
     ...
     <Divider  style={{background: "white"}} align='center'>
            <span className='bg-dark p-tag'>or</span>
     </Divider>
      <div className='my-1'>
     
        <label style={{color: "white", textShadow:"1px 1px black"}} htmlFor="new-color-input">write color:</label>
        <InputText
          id="new-color-input"
          type="text"
          value={newColor}
   ...
   <div className='inline-flex align-items-left'>
      ...
      <ul style={{backdropFilter:"blur(25px)", background: "rgba(0,0,0,0.11)"}}>
        {Bio.colors && Bio.colors.map(color => (
          <li style={{listStyle:"none"}} key={color}>
            <p className='mt-1' style={{ marginBottom:"-5px", width: "50px", height: "50px", background:color}}></p>
 ...
 {  !hideColors && 
         ...
         <img src={profileImg}
            ...
            { !userisopen &&    <h5 style ={{color: "white", textShadow:"1px 1px 0.24px black", fontWeight:"bold"}}> {usernames.useris}  {editable &&  <i onClick={() => setuserisopen(true)} className="pecil text-light p-2 pi pi-pencil"  style={{color:  "white", backgroundColor : "rgba(0,0,0,0.40)" , borderRadius: "20px", cursor: "pointer"}} ></i>}</h5>}
...
<div> <Editor value={Bio.text} onTextChange={(e) => setBio(pre => { return {...pre, text: e.htmlValue }} )} style={{ height: '320px' }} />
 <Button style={{background: "black", height:"fit-content", padding: "3px", paddingRight: "10px"}} label="save" icon="pi pi-check" loading={loading} onClick={updatebio}> </Button>
            ...
            <Postit setLight={setLight} dark={dark} setdark={setdark} setLen={setLength} users = {usernames.useris}/>
...
export const Head = () => <title>Mprofile</title>