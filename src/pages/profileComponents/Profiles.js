...
const [Bio, setBio] = useState({
  ...
  email: '',
        ...
        const followUser = async () => {
          ...
          await app1010.database().ref(`profile/${user.uid}/following/${userId}`).set(true);
          await app1010.database().ref(`profile/${userId}/followers/${user.uid}`).set(true);
...
<h2>Followers</h2>
  ...
  return () => {
    ...
    const unfollowUser = async () => {
     ...
     const updatedFollowers = followers.filter((follower) => follower.id !== user.uid);
     ...
     await app1010.database().ref(`profile/${userId}/followers/${user.uid}`).remove();
    };
...
if(user){
     ...
     await  app1010.database().ref(`profile/${user.uid}/following/${userId}`).remove();
     await app1010.database().ref(`profile/${userId}/following/${user.uid}`).remove();

      // Remove the current user's ID from the "followers" node of the user being unfollowed
     await app1010.database().ref(`profile/${userId}/followers/${user.uid}`).remove();

     await app1010.database().ref(`profile/${user.uid}/followers/${userId}`).remove();

     await app1010.database().ref(`profile/${userId}/blockedMe/${user.uid}`).set(true);
    ...
    if(user){
      const getBlock = async () => {
        ...
        const ref =  app1010.database().ref(`profile/${user.uid}/blockedMe`);
    ref.on('value', (snapshot) => {
      const followerIds = snapshot.val();
  ...
  return () => {
    app1010.database().ref(`profile/${user.uid}/blockedMe`).off('value');
  };
...
const [message,setMessage ] = useState(false)
  ...
  const [mutedme, setMutedme] = useState([])
    const UnBlockUser = async () => {
      ...
      await app1010.database().ref(`profile/${userId}/blockedMe/${user.uid}`).remove();
    // await app1010.database().ref(`profile/${user.uid}/blockList/${userId}`).remove();
     ...
     // const uBlock = blocked.filter((follower) => follower.id !== user.uid);
      ...
      const uBlockme = blockedme.filter((follower) => follower.id !== user.uid);
...
setDeleted( pre => {
  ...
  return () => {
        ...
        const item =[
          {
            label: blockedme && blockedme.some((blocked) =>  blocked.id  ===  user.uid) ? "unBlock" : 'Block',
            icon: 'pi pi-ban', 
           
            command: () => {
              blockedme && blockedme.some((blocked) =>  blocked.id  ===  user.uid) ? UnBlockUser()  :  BlockUser() 
            }
           },
           {
     ...
     let items;
  if( blockedme && blockedme.some((blocked) =>   blocked.id  ===  user.uid) ){
  items = [
    {
      label:   blockedme && blockedme.some((blocked) =>   blocked.id  === user.uid ) ?"unBlock" : 'Block',
      icon: 'pi pi-ban',
  ...
  {
   label:  'mute',
   icon:  mutedme && mutedme.some((mute) =>   mute.id  === user.uid ) ?  'pi pi-volume-on' : 'pi pi-volume-off' ,
   command: () => {
    mutedme && mutedme.some((mute) =>   mute.id  === user.uid ) ? unMuteUser() :   MuteUser() 
  ...
  blocked && blocked.some((blocked) =>   blocked.id  ===  user.uid)
...
if(user){
 ...
 await app1010.database().ref(`profile/${userId.value}/Muted/${user.uid}`).set(true);
...
if(user){
 ...
 await app1010.database().ref(`profile/${userId.value}/Muted/${user.uid}`).remove(true);
 //    await app019.database().ref(`profile/${user.uid}/blockList/${userId.value}`).set(true);
...
<ListBox ref={menu}  breakpoint="767px" value={SelectedFollow} onChange={(e) => setSelectedFollower(e.value)} options={following} optionLabel="name" 
    ...
    <div  className="mb-0">{ReactHtmlParser(htmlString)}</div>
            ...
            <div className="d-flex justify-content-between align-items-center mb-4 " >
              <p className="lead fw-normal mb-0">{Bio.usernamed ? Bio.usernamed : deleted.deletedname && deleted.deletedname}'s_posts</p>
             { Length > 6 && <button className="mb-0 p-0 bg-transparent color-dark"><span className="text-muted">Show all</span></button> }
            </div>

            <Postit  setLight={setLight} dark={dark} setdark={setdark} setLen={setLe} setLength={setLength}  Bio = {Bio.usernamed} userId={userId} />
